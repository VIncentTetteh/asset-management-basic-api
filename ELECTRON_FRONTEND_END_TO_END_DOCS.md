# 🧭 END-TO-END DESKTOP FRONTEND GUIDE (ELECTRON) + COMPLETE API REFERENCE

**Generated**: March 13, 2026  
**Audience**: Coding agents building a desktop frontend in Electron for the Asset Management Enterprise SaaS backend  
**API Version**: v1  
...
This document covers end-to-end desktop frontend integration using Electron.js and includes the **full API reference** (all endpoints, payloads, and responses) for the backend. It is designed so a coding agent can implement the UI, auth flow, data layer, and operational workflows without needing extra context.

---

## 1. Desktop Frontend Architecture (Electron)

### 1.1 Recommended App Structure
...
/electron-app
  ...
  /renderer
    /app               # UI app (React/Vue/Svelte)
    /api               # API client layer
    /state             # Global state (auth/user/org)
    /features          # Feature modules (assets, PO, suppliers, analytics)
  /shared
    models.ts          # DTOs, API types
    config.ts          # base URL, environment flags
```

### 1.2 Security Model

- Keep the **JWT token in the main process** (or encrypted storage) and expose only safe API methods to the renderer via preload.
- Use `contextIsolation: true`, `nodeIntegration: false`.
- All API calls should be executed in the main process (or a dedicated Node service layer) to avoid leaking secrets.

### 1.3 Authentication Flow

1. Renderer collects login credentials.
2. Main process calls `POST /auth/login`.
3. Token is stored securely (OS keychain or encrypted file).
4. Renderer receives a session object (sanitized user info + token expiry).
5. Refresh token flow via `POST /auth/refresh` before expiry.

### 1.4 API Client Design

- Build a single API client with request interceptors for Authorization header.
- Implement retries for idempotent GETs (max 2).
- Use strongly typed response DTOs in `/shared/models.ts`.
- Normalize API errors into a single `AppError` shape.

### 1.5 Core Desktop Features to Implement

- Authentication & session management.
- Asset CRUD + status lifecycle.
...
### 1.6 End-to-End Screens to Build

- Login
- Dashboard
- Assets list + create/edit
- Asset detail with lifecycle history
- Purchase Orders list + create/edit + status updates
- Supplier list + create/edit + verify
...
## 2. End-to-End Workflows (Recommended Implementation Order)

1. **Login + Session**. Implement `/auth/login` and `/auth/refresh`.
2. **Core CRUD**. Assets, Suppliers, Purchase Orders.
3. **Dashboards & Analytics**. Integrate analytics endpoints for charts.
4. **Reporting & Exports**. Use report endpoints for CSV/PDF.
5. **Admin + Monitoring**. Health checks, metrics, notifications, webhooks.

---

## 3. Desktop UX + Data Layer Notes

- Pagination is required for large lists; prefer cursor/page/limit patterns.
- Always show empty states with “Create First” CTA.
- Use optimistic updates for simple status changes.
- Keep audit data viewable via detail pages (created/updated timestamps).

---

## 4. API Reference (Complete)

The section below is the full API reference for the backend. All endpoints include payloads and example responses. Use it directly for frontend integration.
